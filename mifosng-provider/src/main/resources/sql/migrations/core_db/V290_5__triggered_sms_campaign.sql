DELETE FROM `stretchy_report` WHERE `report_name`='Loan Rejected';
...
DELETE FROM `stretchy_report` WHERE `report_name`='Loan Approved';
INSERT INTO `stretchy_report` (`report_name`, `report_type`, `report_subtype`, `report_sql`, `description`, `core_report`, `use_report`)
...
DELETE FROM `stretchy_report` WHERE `report_name`='Loan Repayment';
INSERT INTO `stretchy_report` (`report_name`, `report_type`, `report_subtype`, `report_sql`, `description`, `core_report`, `use_report`)
...
DELETE FROM `r_enum_value` WHERE `enum_name`='loan_type_enum';
...
DELETE FROM `stretchy_parameter` WHERE `parameter_name`='DefaultLoan';
...
VALUES ('DefaultLoan', 'loanId', 'Loan', 'none', 'number', '-1', 'Y', 'select ml.id \nfrom m_loan ml \nleft join m_client mc on mc.id = ml.client_id \nleft join m_office mo on mo.id = mc.office_id \nwhere mo.id = ${officeId} or ${officeId} = -1');
DELETE FROM `stretchy_parameter` WHERE `parameter_name`='DefaultClient';
INSERT INTO `stretchy_parameter` (`parameter_name`, `parameter_variable`, `parameter_label`, `parameter_displayType`, `parameter_FormatType`, `parameter_default`, `selectAll`, `parameter_sql`)
VALUES ('DefaultClient', 'clientId', 'Client', 'none', 'number', '-1', 'Y', 'select mc.id \nfrom m_client mc\n left join m_office on mc.office_id = mo.id\n where mo.id = ${officeId} or ${officeId} = -1');
DELETE FROM `stretchy_parameter` WHERE `parameter_name`='DefaultGroup';
INSERT INTO `stretchy_parameter` (`parameter_name`, `parameter_variable`, `parameter_label`, `parameter_displayType`, `parameter_FormatType`, `parameter_default`, `selectAll`, `parameter_sql`)
VALUES ('DefaultGroup', 'groupId', 'Group', 'none', 'number', '-1', 'Y', 'select mg.id \nfrom m_group mg\nleft join m_office mo on mg.office_id = mo.id\nwhere mo.id = ${officeId} or ${officeId} = -1');
DELETE FROM `stretchy_parameter` WHERE `parameter_name`='SelectLoanType';
INSERT INTO `stretchy_parameter` (`parameter_name`, `parameter_variable`, `parameter_label`, `parameter_displayType`, `parameter_FormatType`, `parameter_default`, `selectAll`, `parameter_sql`)
VALUES ('SelectLoanType', 'loanType', 'Loan Type', 'select', 'number', '-1', 'Y', 'select -1 as id, "All" as value\nUNION\n(select\nenum_id as id,\nenum_value as value\nfrom r_enum_value\nwhere enum_name = \'loan_type_enum\')');

SET @officeId = (select id from stretchy_parameter where `parameter_name` = 'OfficeIdSelectOne');
UPDATE `stretchy_parameter` SET `parent_id` = @officeId where `parameter_name` = 'DefaultLoan';
UPDATE `stretchy_parameter` SET `parent_id` = @officeId where `parameter_name` = 'DefaultClient';
UPDATE `stretchy_parameter` SET `parent_id` = @officeId where `parameter_name` = 'DefaultGroup';

SET @loanReject = (select id from stretchy_report where report_name = 'Loan Rejected');
SET @loanApprove = (select id from stretchy_report where report_name = 'Loan Approved');
SET @loanRepay = (select id from stretchy_report where report_name = 'Loan Repayment');
SET @staffId = (select id from stretchy_parameter where parameter_name = 'LoanOfficerSelectOneRec');
SET @defaultLoan = (select id from stretchy_parameter where parameter_name = 'DefaultLoan');
SET @defaultClient = (select id from stretchy_parameter where parameter_name = 'DefaultClient');
SET @defaultGroup = (select id from stretchy_parameter where parameter_name = 'DefaultGroup');
SET @loanType = (select id from stretchy_parameter where parameter_name = 'SelectLoanType');

DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanReject AND `parameter_id`=@officeId;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanApprove AND `parameter_id`=@officeId;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanRepay AND `parameter_id`=@officeId;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanReject AND `parameter_id`=@staffId;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanApprove AND `parameter_id`=@staffId;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanRepay AND `parameter_id`=@staffId;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanApprove AND `parameter_id`=@defaultLoan;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanReject AND `parameter_id`=@defaultLoan;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanApprove AND `parameter_id`=@defaultClient;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanReject AND `parameter_id`=@defaultClient;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanApprove AND `parameter_id`=@defaultGroup;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanReject AND `parameter_id`=@defaultGroup;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanReject AND `parameter_id`=@loanType;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanApprove AND `parameter_id`=@loanType;
DELETE FROM `stretchy_report_parameter` WHERE `report_id`=@loanRepay AND `parameter_id`=@loanType;

INSERT INTO `stretchy_report_parameter` (`report_id`, `parameter_id`, `report_parameter_name`)
VALUES (@loanReject, @officeId, 'Office'),
(@loanApprove, @officeId, 'Office'),
(@loanRepay, @officeId, 'Office'),
(@loanReject, @staffId, 'loanOfficer'),
(@loanApprove, @staffId, 'loanOfficer'),
(@loanRepay, @staffId, 'loanOfficer'),
(@loanReject, @defaultLoan, 'loanId'),
(@loanApprove, @defaultLoan, 'loanId'),
(@loanReject, @defaultClient, 'clientId'),
(@loanApprove, @defaultClient, 'clientId'),
(@loanReject, @defaultGroup, 'groupId'),
(@loanApprove, @defaultGroup, 'groupId'),
(@loanReject, @loanType, 'loanType'),
(@loanApprove, @loanType, 'loanType'),
(@loanRepay, @loanType, 'loanType');
