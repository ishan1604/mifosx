...
public interface LoanTransactionRepository extends JpaRepository<LoanTransaction, Long>, JpaSpecificationExecutor<LoanTransaction> {
    ...
    public static final String FIND_ALL_TRANSACTIONS_AFTER_CLIENT_TRANSFER = "from LoanTransaction t1 where t1.loan.id = :loanId and " +
            "t1.id > (select max(t2.id) from LoanTransaction t2 where t2.loan.id = :loanId and t2.typeOf = :enumType)";
