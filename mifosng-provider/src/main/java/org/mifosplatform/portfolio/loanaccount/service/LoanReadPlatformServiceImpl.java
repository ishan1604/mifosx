...
public class LoanReadPlatformServiceImpl implements LoanReadPlatformService {
    ...
    public Collection<OverdueLoanScheduleData> retrieveAllOverdueLoans() {
        ...
        final String sql = "select " + rm.schema() +
                ...
                " and plc.charge_id not in (SELECT charge_id from m_loan_charge as mlc where mlc.loan_id = ml.id and mlc.charge_id = plc.charge_id and mlc.due_for_collection_as_of_date > fromdate AND mlc.due_for_collection_as_of_date <= duedate) AND datediff(ls.duedate,ls.fromdate) > 0";
